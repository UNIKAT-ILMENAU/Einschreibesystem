...
class WorkshopController extends FOSRestController implements ClassResourceInterface
{
    ...
    public function patchAction($id, ParamFetcher $paramFetcher)
    {
        ...
        $template = $this->getDoctrine()->getRepository("CoreEntityBundle:EmailTemplate")->findOneBy(
            ['template_name' => 'Changed Workshopdetails']
        );
        if (!$template) {
            ...
            $message = \Swift_Message::newInstance()
                ...
                ->setBody(
                    $renderTemplate->render(
                        ["workshop" => $workshop, "participant" => $wp->getParticipant(), 'url' => $url]
                    ),
                    'text/html'
                );
    ...
    public function deleteAction($id)
    {
        ...
        foreach ($workshopParticipants as $wp) {
            $message = \Swift_Message::newInstance()
                ...
                ->setTo($wp->getParticipant()->getEmail())
                ->setBody(
                    $renderTemplate->render(["workshop" => $workshop, "participant" => $wp->getParticipant()]),
                    'text/html'
                );
            $this->get('mailer')->send($message);
    ...
    public function patchWaitinglistAction($id, $participantId)
        /**Workshop ID!, Workshopüberbuchung: von der Warteliste auf die Nichtwarteliste*/
    {
        ...
        if (!$workshopParticipant) {
            return $this->handleView(
                $this->view(['code' => 404, 'message' => "No Participants on waiting list found"], 404)
            );
        }
        $workshopParticipant->setWaiting(0);
        /** 0 -> im Workshop, 1-> Waiting */
    ...
    {
        $workshopParticipant = $this->getDoctrine()->getRepository("CoreEntityBundle:WorkshopParticipants")->findOneBy(
            ["workshop" => $id, "participant" => $participantId]
        );
    ...
    public function getParticipantsAction($id)
    {
        ...
        foreach ($participantsList as $p) {
            $list[] = [
                ...
                'surname'      => $p->getParticipant()->getSurname(),
                'email'        => $p->getParticipant()->getEmail(),
                'participated' => $p->isParticipated()
            ];
